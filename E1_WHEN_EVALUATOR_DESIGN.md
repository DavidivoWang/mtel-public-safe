...
`inspect_mtel` compiles every expression and returns `ERROR` for unsafe or invalid expressions. `run_mtel` uses a dependency-free runtime validator kept congruent with the packaged JSON Schema before evaluation.
